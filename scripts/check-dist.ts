...
const temporary = mkdtempSync(join(tmpdir(), "cail-identity-dist-"));
...
try {
  ...
  if (JSON.stringify(expectedFiles) !== JSON.stringify(actualFiles)) {
    throw new Error("cail-identity: dist file list does not match source build");
  }
  for (const file of expectedFiles) {
    ...
    if (!matches) {
      throw new Error(`cail-identity: dist/${file} does not match source build`);
    }
  }
} finally {
